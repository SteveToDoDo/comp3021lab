...
public class Account {
	...
	public static Consumer<Account> add100 = f->f.balance+=100;
	...
	public static Predicate<Account> checkBound = a->a.lowerBound.test(a)&&a.upperBound.test(a);
	...
	// TODO: Task3 
	// replace the null with a lambda expression
	public static AddMaker maker = (int N) -> (Account ac)->{ac.balance = ac.balance + N;}; 
	...
	public static int getMaxAccountID(List<Account> accounts) {
		// TODO: Task4 
		// replace the null with a lambda expression
		Account maxOne = accounts.stream().reduce(new Account(0, -100), (max,curr)-> curr.balance>max.balance?curr:max);
				//(max,current) -> {if (current.balance>max.balance) {max.balance=current.balance;max.id=current.balance;}});
